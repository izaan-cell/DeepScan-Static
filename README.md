...
The marketing/download site for [DeepScan](https://github.com/izaan-cell/DeepScan), a free
AI search app for your files. This repo is intentionally separate from the app itself: it's
a plain static site (no build step, no framework) deployed to Vercel, while the actual app
(Rust + Go + Java) is downloaded and runs entirely on the user's own machine.
